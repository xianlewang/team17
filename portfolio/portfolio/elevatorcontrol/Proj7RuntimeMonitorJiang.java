...
public class Proj7RuntimeMonitorJiang extends RuntimeMonitor {
    ...
    int ReversalCount = 0;
    public Proj7RuntimeMonitorJiang() {
    	for (int i = 0; i < 8; i++) {
    ...
    private class DriveStateMachine {
    	...
    	int carPosition = 0;
    	int curFloor = -1; // only use when car stop at a certain floor
    	Speed curSpeed = Speed.STOP;
    	Direction curDir = Direction.STOP;
    	...
    	public void receive(ReadableCarLevelPositionPayload msg) {
    		carPosition = msg.position();
    		if(curDir == Direction.UP && carPosition < 35000) {
    			nextFloorHeight = (carPosition/5000 + 1) * 5000;
    		}
    		else if(curDir == Direction.DOWN) {
    			nextFloorHeight = (carPosition/5000) * 5000;
    		}    		
    	}    	
    	public void receive(ReadableDrivePayload msg) {
    		curSpeed = msg.speed();
    		curDir = msg.direction();    		
    	}
    	...
    	private void updateState() {
            ...
            if(curDir == Direction.UP) {
            	noCall = HallCall[curFloor-1][0][0] || HallCall[curFloor-1][1][0] || CarCall[curFloor-1][0] || CarCall[curFloor-1][1];
            }
            else if(curDir == Direction.DOWN) {
            	noCall = HallCall[curFloor-1][0][1] || HallCall[curFloor-1][1][1] || CarCall[curFloor-1][0] || CarCall[curFloor-1][1];
            }
                       
            if(curSpeed == Speed.STOP) {
            	if(noCall) {
            ...
            else if(Math.abs(nextFloorHeight - carPosition) > 1000 && curSpeed == Speed.SLOW) {
            	newState = DriveState.MOVING_TOO_SLOW;
            }
            else {
            	newState = DriveState.MOVING_NORMAL;
            }
            
            if(newState != previousState) {
            	switch(newState) {
            		case MOVING_TOO_SLOW:
            			warning("WARNING: Moving too slow!");
            			break;
            		case NO_CALL_STOP:
            			warning("WARNING: Stop at no pending call floor!");
            			break;
            		default:
            			break;
