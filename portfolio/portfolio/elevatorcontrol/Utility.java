...
public class Utility {
    ...
    public static class CallArray {
        ...
        public int size() {
            ...
            for (int i : carIndexList) {
                if (carCallArray.get(i).getValue()) {
                	//System.out.println("Find Car Call");
                    ++sum;
                }
            }
            for (int i : hallIndexList) {
                if (hallCallArray.get(i).getValue()) {
                	//System.out.println("Find Hall Call");
                    ++sum;
                }
            }
            return sum;
        ...
        public boolean isCalled(int f, Direction d) {
            ...
            int iCarFront = ReplicationComputer.computeReplicationId(f, Hallway.FRONT);
            int iCarBack = ReplicationComputer.computeReplicationId(f, Hallway.BACK);
            if (d == Direction.STOP) {// only look for car call
                if (carCallArray.get(iCarFront).getValue() || carCallArray.get(iCarBack).getValue()) {
                    return true;
                }
            } else {
                int iHallFront = ReplicationComputer.computeReplicationId(f, Hallway.FRONT, d);
                int iHallBack = ReplicationComputer.computeReplicationId(f, Hallway.BACK, d);
                if (hallCallArray.get(iHallFront).getValue() || hallCallArray.get(iHallBack).getValue()) {
                    return true;
                }
            }
            return false;
        ...
        public boolean isCalled(int f, Direction d, Hallway b) {
            int iCar = ReplicationComputer.computeReplicationId(f, b);
            if (carCallArray.get(iCar).getValue()) {
                return true;
            }
            int iHall = ReplicationComputer.computeReplicationId(f, b, d);
